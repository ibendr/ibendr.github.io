...
function chessPieceAvailableMoves() {
  ...
  if ( this.nam == 'P' ) {// pawn - special case
    ...
    if ( j == ( team ? 0 : board.n - 1 ) ) {
      // promotion - perhaps we'll deal with it elsewhere
    }
  }
...
function chessSetSetTargets( it ) {
	playTeam = it.toPlay
	while ( it.pieces.iter( "piece" ) ) {
// 		aalert( ["B","W"][ piece.team ] + piece.nam + piece.square.i + piece.square.j )
		piece.dropTargets = ( piece.team == playTeam ) ? piece.availableMoves() : []
		}
// 	alert('done')
	}
...
// 	alert(t.nam + ' moved to ' + t.dropTargetHit.i + " , " + t.dropTargetHit.j)
	if ( t.availableMoves ) {
		...
		// check if en-passant
// 		if ( t
		// check if capture - well, empty the square anyway,  killing occupants
		chessSetKillPieces( t.square , t.set )
