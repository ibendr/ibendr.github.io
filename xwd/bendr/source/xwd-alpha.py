...
class xwd( object ):
  ...
  def innerAnalyse( I ):
	...
	# NOW in alpha version, 'let in' words are still onlist
	# of spots to check, so we'll 'let them in' again by
	# noticing them on the 'words used' list
	if regExp in wordLists[ len( regExp ) ] or regExp in I.wordsUsed:
	  newList = [ regExp ]
	else:
	  newList = [ ]
      else:
  ...
  def setNewSpotContent( I , sp , newList , 
	...
	if len( newSpotContent[ i ] ) < len( I.cellContent[ cl ] ):
	  ...
	  if culls:
	    ...
	    # TODO: HERE'S WHERE WE MAKE IT ALPHABETIC
	    # Check if it's a head cell (NOT just i==0 ?)
	    # If so , 
	    #  - if down to one possibility, cull it from others
	    #  - check that collective possibilities cover A-Z
	    cls2 = [ cl ]
	    if cl in I.headCells:
	      cls1 = I.checkHeadCells( cl )
	      if not cls1:
		# Contradiction raised by headcell restrictions
		I.contradiction = ( sp , str( cl ) + "-alpha" )
		I.spotsToAdjust.clear( )
		return
	      if cls1 != True:
		# More cells to check - 
		cls2 += cls1
	    # Now mark other spot(s) through cell for adjustment
	    for cl2 in cls2:
	      for ( sp1, i1 ) in cl2.spots:
		if not sp1 == sp: # just to save time
		  if sp1 in I.liveSpots: # shouldn't be necessary?
		    I.spotsToAdjust.add( sp1 )

      # Mark this spot as done

    I.spotsToAdjust.discard( sp )
  def checkHeadCells( I , cl = None ):
    """
    Check whether the head cells fit with alpha restrictions,
    i.e. no two forced to be same letter, and
    all letters able to occur somewhere.
    If cl is specified, we assume only it / they changed since
    ...
    newChanges = set( )
    while changes:
      # check changed cells for being down to one possibility
    ...
    if cover < set( ABC ):
      return False
    return list( newChanges ) or True
